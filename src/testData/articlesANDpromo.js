import imgPromo2 from "../img/test/timthumb.jpg";
import imgPromo3 from "../img/test/imgPromo3.jpg";
import imgPromo1 from "../img/test/breakingPrices.jpg";

const data = [
  {
    id: 1,
    title: 'Ломаем цены! 1',
    description: 'C 1 по 31 октября очень низкие цены на необходимые препараты.',
    image: imgPromo2,
    date: '1 октября 2020 г.'
  },
  {
    id: 2,
    title: 'Опять скидки! 2',
    description: 'Снова скидки на полезные товары для здоровья!',
    image: imgPromo3,
    date: '2 cентября 2020 г.'
  },
  {
    id: 3,
    title: 'Опять скидки! 3',
    description: 'Снова скидки на полезные товары для здоровья!',
    image: imgPromo3,
    date: '2 cентября 2020 г.'
  },
  {
    id: 4,
    title: 'Опять скидки! 4',
    description: 'Снова скидки на полезные товары для здоровья!',
    image: imgPromo1,
    date: '2 cентября 2020 г.'
  },
  {
    id: 5,
    title: 'Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены!',
    description: 'C 1 по 31 октября очень низкие цены на необходимые препараты. C 1 по 31 октября очень низкие цены на необходимые препараты. C 1 по 31 октября очень низкие цены на необходимые препараты.',
    image: imgPromo2,
    date: '1 октября 2020 г.'
  },
  {
    id: 6,
    title: 'Ломаем цены! 6',
    description: 'C 1 по 31 октября очень низкие цены на необходимые препараты.',
    image: imgPromo2,
    date: '1 октября 2020 г.'
  },
  {
    id: 7,
    title: 'Вегетарианство: польза или вред? 7',
    description: 'Более миллиарда людей по всему миру выбирают для себя путь вегетарианства, и с каждым днём таких людей становится всё больше.',
    image: 'https://xn--n1aalg.xn--80ae2aeeogi5fxc.xn--p1ai/article/2efae72b6523169bca5c91d237363e3a.jpg',
    date: '1 октября 2020 г.'
  },
  {
    id: 8,
    title: 'Как правильно ухаживать за кожей лица? 8',
    description: 'Пришло время позаботиться о коже!',
    image: 'https://xn--n1aalg.xn--80ae2aeeogi5fxc.xn--p1ai/article/b0855ca6f18d9187a5567aad203f4c45.jpg',
    date: '1 октября 2020 г.'
  },
  {
    id: 9,
    title: 'Приятный запах в доме: 8 советов 9',
    description: 'Мы собрали для вас несколько советов, которые помогут вам устранить возможные источники неприятных запахов и добавить свежести в свой дом.',
    image: 'https://xn--n1aalg.xn--80ae2aeeogi5fxc.xn--p1ai/article/4554b91c015f4ee8c439ceee307e034f.jpg',
    date: '1 октября 2020 г.'
  },
  {
    id: 10,
    title: 'Очки с поляризацией: кому и зачем нужны? 10',
    description: 'Чем поляризационные очки отличаются от других и действительно ли они необходимы?',
    image: 'https://xn--n1aalg.xn--80ae2aeeogi5fxc.xn--p1ai/article/84fa33cede7d34e0567978778f8aa0b6.jpg',
    date: '1 октября 2020 г.'
  },
  {
    id: 11,
    title: 'Ломаем цены! 11',
    description: 'C 1 по 31 октября очень низкие цены на необходимые препараты.',
    image: imgPromo2,
    date: '1 октября 2020 г.'
  },
  {
    id: 12,
    title: 'Вегетарианство: польза или вред? 12',
    description: 'Более миллиарда людей по всему миру выбирают для себя путь вегетарианства, и с каждым днём таких людей становится всё больше.',
    image: 'https://xn--n1aalg.xn--80ae2aeeogi5fxc.xn--p1ai/article/2efae72b6523169bca5c91d237363e3a.jpg',
    date: '1 октября 2020 г.'
  },
  {
    id: 13,
    title: 'Как правильно ухаживать за кожей лица? 13',
    description: 'Пришло время позаботиться о коже!',
    image: 'https://xn--n1aalg.xn--80ae2aeeogi5fxc.xn--p1ai/article/b0855ca6f18d9187a5567aad203f4c45.jpg',
    date: '1 октября 2020 г.'
  },
  {
    id: 14,
    title: 'Приятный запах в доме: 8 советов 14',
    description: 'Мы собрали для вас несколько советов, которые помогут вам устранить возможные источники неприятных запахов и добавить свежести в свой дом.',
    image: 'https://xn--n1aalg.xn--80ae2aeeogi5fxc.xn--p1ai/article/4554b91c015f4ee8c439ceee307e034f.jpg',
    date: '1 октября 2020 г.'
  },
  {
    id: 15,
    title: 'Очки с поляризацией: кому и зачем нужны? 15',
    description: 'Чем поляризационные очки отличаются от других и действительно ли они необходимы?',
    image: 'https://xn--n1aalg.xn--80ae2aeeogi5fxc.xn--p1ai/article/84fa33cede7d34e0567978778f8aa0b6.jpg',
    date: '1 октября 2020 г.'
  },
  {
    id: 16,
    title: 'Ломаем цены! 16',
    description: 'C 1 по 31 октября очень низкие цены на необходимые препараты.',
    image: imgPromo2,
    date: '1 октября 2020 г.'
  },
  {
    id: 17,
    title: 'Опять скидки! 17',
    description: 'Снова скидки на полезные товары для здоровья!',
    image: imgPromo3,
    date: '2 cентября 2020 г.'
  },
  {
    id: 18,
    title: 'Опять скидки! 18',
    description: 'Снова скидки на полезные товары для здоровья!',
    image: imgPromo3,
    date: '2 cентября 2020 г.'
  },
  {
    id: 19,
    title: 'Опять скидки! 19',
    description: 'Снова скидки на полезные товары для здоровья!',
    image: imgPromo1,
    date: '2 cентября 2020 г.'
  },
  {
    id: 20,
    title: 'Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! Ломаем цены! 20',
    description: 'C 1 по 31 октября очень низкие цены на необходимые препараты. C 1 по 31 октября очень низкие цены на необходимые препараты. C 1 по 31 октября очень низкие цены на необходимые препараты.',
    image: imgPromo2,
    date: '1 октября 2020 г.'
  },
]

export default data